...
putHeader plen xid = putWord8 version >> putWord8 tp >> putWord16be len >> putWord32be xid
  where version = 0x01
        tp      = (fromIntegral $ fromEnum OFPT_PACKET_IN)
        len     = (8 + 8 + 16 + 32) `div` 8 + fromIntegral plen
...
testEth fs srcMac = do
  let cargo = [0x45,0x00,0x00,0x32,0x00,0x00,0x00,0x00,
               ...
               0x00,0x36]
  return $ EthFrame { destMAC    = 0x800000000000 .|. (fromIntegral fs)
...
--    putStrLn "AAAA"  
   ...
   -- putStrLn "BBBB"  
    ...
    pl  <- liftM (encodePutM.putEthernetFrame) (testEth sw srcMac)
    atomically $ writeTBMChan chan $! bsStrict $ runPut $ putOfpMsg pl (fromIntegral j)
